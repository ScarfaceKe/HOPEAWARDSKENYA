...
export async function runMigrations() {
  ...
  try {
    ...
    `);
    await client.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS kcb_transactions_checkout_request_id_unique
        ON kcb_transactions (checkout_request_id);
    `);
    await client.query(`
      CREATE INDEX IF NOT EXISTS kcb_transactions_status_idx
        ON kcb_transactions (status);
    `);
    await client.query(`
